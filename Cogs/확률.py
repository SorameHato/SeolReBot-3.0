...
class _prob(commands.Cog): #prob은 probability의 약자
    ...
    @commands.slash_command(name='확률',guild_ids = guild_ids, description="무언가가 일어날 확률이 궁금하시다고요? 유설레가 계산해드릴게요!")
    async def prob(self, ctx,text:discord.Option(str,'어떤 것의 확률을 계산할 지 입력해주세요. (예시 : 하토가 귀엽지 않을, 하토가 귀엽지 않을 확률, 하토가 내일 오락을 갈 가능성)',name='문장')):
