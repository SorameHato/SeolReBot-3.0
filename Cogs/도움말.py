...
class _도움말(commands.Cog):
    ...
    async def 도움말(self,ctx,*,text='0'):
        ...
        if(num == '1' or num == '0' or num == ''):
            ...
            embed.add_field(name='.설레 도움말 [페이지]',value='명령어 목록을 볼 수 있는 명령어에요! 지금 쓰고 있는 명령어에요.',inline = False)
            embed.add_field(name='.설레 정보',value='설레봇의 정보를 볼 수 있어요! 설레봇의 버전, 봇이 시작된 시간 등을 볼 수 있어요.',inline = False)
            embed.add_field(name='.설레 소개',value='설빈레피딕스와 설빈레피딕스 마스코트인 저, 유설레의 간단한 소개를 볼 수 있어요.',inline = False)
            embed.add_field(name='.설레 확률 [문장]',value='어떤 것에 대한 확률을 구할 수 있는 명령어에요. 흔히들 \'확률봇\'이라고 많이 부르는 것 같아요!',inline = False)
            embed.set_footer(text='설레봇 버전 {}'.format(self.bot.srver))
            await ctx.send('설레봇의 명령어 목록입니다!(1/3)',embed=embed)
        elif(num == '2'):
            embed = discord.Embed(color=0x04ccff)
