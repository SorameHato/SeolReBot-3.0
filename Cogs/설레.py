...
class _설레(commands.Cog):
    ...
    async def 약어조회(self,ctx,text:discord.Option(str,'조회하고 싶으신 약어의 종류를 선택해주세요.',name='종류',choices=['도시철도','라이너'],required=True)):
        ...
        if(text=='도시철도'):
            ...
            dummyMetroResultText = tui.fixedWidth('등급명',9,1)
            dummyMetroResultText += tui.fixedWidth('약어',5,0)
            dummyMetroResultText += tui.fixedWidth('전광판',6,2)
            for metroArray in dummyMetroList:
                dummyMetroResultText += '\n'
                dummyMetroResultText += tui.fixedWidth(metroArray[0],9,0)
                dummyMetroResultText += tui.fixedWidth(metroArray[1],5,0)
                dummyMetroResultText += tui.fixedWidth(metroArray[2],6,2)
            ...
            dummyLinerResultText = tui.fixedWidth('라이너명',30,1)
            dummyLinerResultText += tui.fixedWidth('약어',6,0)
            dummyLinerResultText += tui.fixedWidth('전광판',6,2)
            for linerArray in dummyLinerList:
                dummyLinerResultText += '\n'
                dummyLinerResultText += tui.fixedWidth(linerArray[0],30,0)
                dummyLinerResultText += tui.fixedWidth(linerArray[1],6,0)
                dummyLinerResultText += tui.fixedWidth(linerArray[2],6,2)
